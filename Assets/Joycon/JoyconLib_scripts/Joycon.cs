...
public class Joycon
{
	...
	public Quaternion GetVector()
	{
		...
		var orientation = q * Quaternion.LookRotation(v1, v2);
		return v2 != Vector3.zero ? orientation : Quaternion.identity;
	}
		...
		if (state > state_.NO_JOYCONS)
		{
			...
			while (reports.Count > 0)
			{
				...
				if (reports != null)
					...
					{
						DebugPrint(
							$"Dequeue. Queue length: {reports.Count:d}. Packet ID: {report_buf[0]:X2}. Timestamp: {report_buf[1]:X2}. Lag to dequeue: {DateTime.Now.Subtract(rep.GetTime()).ToString()}. Lag between packets (expect 15ms): {rep.GetTime().Subtract(ts_prev).ToString()}",
							DebugType.THREADING);
					}
